...
class Relay {
  ...
  getListener(subscribeId: string) {
    return this.listeners.find((listener: Listener) =>
      listener.subscribeId === subscribeId
    );
  }
  ...
  sendEvent(event: NostrEvent) {
    return new Promise((resolve, reject) => {
      ...
      this.nostr.once(
        "relayPost",
        (id: string, status: boolean, errorMessage: string, relay: Relay) => {
