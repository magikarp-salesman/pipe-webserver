...
import * as secp from "https://deno.land/x/secp256k1@1.7.1/mod.ts";
...
declare interface Nostr {
  on<U extends keyof NostrEvents>(
    event: U,
    listener: NostrEvents[U],
  ): this;

  emit<U extends keyof NostrEvents>(
    event: U,
    ...args: Parameters<NostrEvents[U]>
  ): boolean;
}
...
class Nostr extends EventEmitter {
  ...
  public set privateKey(value: string) {
    ...
    if (value) {
      ...
      this._publicKey = decoder.decode(
        mod.encode(secp.schnorr.getPublicKey(this._privateKey)),
      );
    }
  }
  ...
  filter(filters: NostrFilters, unique = true) {
    ...
    return {
      ...
      async *[Symbol.asyncIterator]() {
        function indexPromise<T>(
          p: Promise<T>,
          i: number,
        ): Promise<{ value: T; i: number }> {
          return new Promise((resolve, reject) =>
            p.then((r) => resolve({ value: r, i })).catch((reason) =>
              reject({ reason, i })
            )
          );
        }
        ...
        const indexedPromises: Array<
          Promise<{ value: IteratorResult<NostrEvent>; i: number }>
        > = nextPromises.map((p, i) => indexPromise(p, i));
        ...
        while (relayIterators.length > 0) {
          ...
          if (indexResult.value.done) {
            ...
            if (
              !unique ||
              (yieldedEventIds.indexOf(indexResult.value.value.id) === -1)
            ) {
            ...
            indexedPromises[indexResult.i] = indexPromise(
              relayIterators[indexResult.i].next(),
              indexResult.i,
            );
          }
        }
  ...
  async globalFeed({
    ...
    since,
    authors,
  }: { limit?: number; since?: number; authors?: Array<string> }): Promise<
    Array<NostrPost>
  > {
    const filters = {
  ...
  private async sendPost(
    ...
    for (const relay of this.relayInstances) {
      try {
        event.tags = event.tags.map((tags: Array<string>) =>
          tags[0] === "e" ? [tags[0], tags[1], relay.url, tags[3]] : tags
        );
        event.id = await this.calculateId(event);
        event.sig = new TextDecoder().decode(
          mod.encode(await this.signId(event.id)),
        );
        this.log("Send event;", event);
        await relay.sendEvent(event);
      } catch (err: any) {
        console.error(
          `Send event error; ${err.message} Relay name; ${relay.name}`,
        );
      }
    }
  }
  ...
  public async sendMessage(to: string, message: any) {
    ...
    const encrypted = await Message.encryptMessage(
      to,
      message,
      this._privateKey,
    );
    ...
    for (const relay of this.relayInstances) {
      try {
        ...
        console.error(
          `Send direct message event error; ${err.message} Relay name; ${relay.name}`,
        );
      }
    }
  }
  ...
  public async getMessages(): Promise<NostrMessage[]> {
    ...
    for (const event of events) {
      ...
      try {
        ...
        const msg = await Message.decrypt(
          this._privateKey,
          pubKey,
          event.content,
        );
